...
public class TestLibrary {
    ...
    /**
     * Routines that enable rmi tests to fail in a uniformly informative
     * fashion.
     */
    ...
    /**
     * Routine that "works-around" a limitation in jtreg. Currently it is not
     * possible for a test to specify that the test harness should build a given
     * source file and install the resulting class in a location that is not
     * accessible from the test's classpath. This method enables a test to move
     * a compiled test class file from the test's class directory into a given
     * "codebase" directory. As a result the test can only access the class file
     * for <code>className</code>if the test loads it from a classloader (e.g.
     * RMIClassLoader).
     * 
     * Tests that use this routine must have the following permissions granted
     * to them:
     * 
     * getProperty user.dir getProperty etc.
     */
    public static URL installClassInCodebase(String className, String codebase)
            throws MalformedURLException {
        return installClassInCodebase(className, codebase, true);
    }

    public static URL installClassInCodebase(String className, String codebase,
            boolean delete) throws MalformedURLException {
        /*
         * NOTES/LIMITATIONS: The class must not be in a named package, and the
         * codebase must be a relative path (it's created relative to the
         * working directory).
         */
        String classFileName = className + ".class";

        /*
         * Specify the file to contain the class definition. Make sure that the
         * codebase directory exists (underneath the working directory).
         */

        String currentProjectLoc = System.getProperty("user.dir");

        File dstDir = (new File(currentProjectLoc, codebase));
        ...
        /*
         * Specify where we will copy the class definition from, if necessary.
         * After the test is built, the class file can be found in the
         * "test.classes" directory.
         */
        File srcDir = new File(currentProjectLoc + "/bin/testUtils");
        ...
        /*
         * If the class definition is not already located at the codebase, copy
         * it there from the test build area.
         */
        if (!dstFile.exists()) {
            if (!srcFile.exists()) {
                throw new RuntimeException(
        ...
        /*
         * After the class definition is successfully installed at the codebase,
         * delete it from the test's CLASSPATH, so that it will not be found
         * there first before the codebase is searched.
         */
        if (srcFile.exists()) {
    ...
    /**
     * Allow test framework to control the security manager set in each test.
     * 
     * @param managerClassName
     *            The class name of the security manager to be instantiated and
     *            set if no security manager has already been set.
     */
    public static void suggestSecurityManager(String managerClassName) {
        ...
        if (System.getSecurityManager() == null) {
            try {
                ...
                manager = ((SecurityManager) Class.forName(managerClassName)
                        .newInstance());
            } catch (ClassNotFoundException cnfe) {
                bomb("Security manager could not be found: " + managerClassName,
                        cnfe);
            } catch (Exception e) {
    ...
    /**
     * Method to capture the stack trace of an exception and return it as a
     * string.
     */
    ...
    /**
     * Returns extra test properties. Looks for the file "../../test.props" and
     * reads it in as a Properties file. Assuming the working directory is
     * "<path>/JTwork/scratch", this will find "<path>/test.props".
     */
    ...
    /**
     * Returns an extra test property. Looks for the file "../../test.props" and
     * reads it in as a Properties file. Assuming the working directory is
     * "<path>/JTwork/scratch", this will find "<path>/test.props". If the
     * property isn't found, defaultVal is returned.
     */
